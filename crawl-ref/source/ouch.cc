        ...
        || flavour == BEAM_STEAM)
    {
        if (you.mutation[MUT_ICEMAIL])
        {
            if (!you.duration[DUR_ICEMAIL_DEPLETED])
                mpr_nojoin(MSGCH_DURATION, jtrans("Your icy envelope dissipates!"));
    ...
    case BEAM_GHOSTLY_FLAME:
    {
        if (you.holiness() == MH_UNDEAD)
        {
            if (doEffects && hurted > 0)
            {
                ...
                mpr(jtrans("You are bolstered by the flame."));
            }
            hurted = 0;
        }
        else
        {
            ...
            {
                mpr(jtrans("The flames sap you greatly!"));
                xom_is_stimulated(200);
            }
        }
    }
...
static void _lose_level_abilities()
{
        ...
        && !you.wearing_ego(EQ_ALL_ARMOUR, SPARM_FLYING))
    {
        ...
        mpr_nojoin(MSGCH_WARN, jtrans("You feel your flight won't last long."));
    }
}
                                  ...
                                  mid_t death_source)
{
    ...
    if (how_many > 0)
    {
        ...
        if (count_created > 0)
        {
            ...
            {
                mpr_nojoin(MSGCH_GOD, jtrans("A shower of butterflies erupts from you!"));
                take_note(Note(NOTE_XOM_EFFECT, you.piety, -1, jtrans("butterfly on damage")), true);
            }
            else
            {
                mprf(jtransc("You shudder from the %s and a %s!"),
                     jtransc(count_created > 1 ? "flood of jellies pours out from you"
                                               : "jelly pops out"));
            }
        }
    }
...
static void _maybe_fog(int dam)
{
    ...
    {
        mpr_nojoin(MSGCH_GOD, jtrans("You emit a cloud of colourful smoke!"));
          ...
          bool see_source, const char *death_source_name)
{
    ...
    if (dam != INSTANT_DEATH)
    {
        ...
        {
            simple_god_message(jtransc(" protects you from harm!"));
            return;
        }
        ...
        if (dam > 0 && you.hp_max <= dam * 2)
            mpr_nojoin(MSGCH_DANGER, jtrans("Ouch! That really hurt!"));
        ...
        if (you.hp > 0 && dam > 0)
        {
                ...
                && (death_type != KILLED_BY_POISON || poison_is_lethal()))
            {
                ...
                mpr_nojoin(MSGCH_DANGER, jtrans("* * * LOW HITPOINT WARNING * * *"));
