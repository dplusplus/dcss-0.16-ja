...
void manage_fire_shield(int delay)
{
    ...
    {
        mpr_nojoin(MSGCH_DURATION, jtrans("Your ring of flames gutters out."));
        return;
    }
    ...
    if (old_dur > threshold && you.duration[DUR_FIRE_SHIELD] < threshold)
        mpr_nojoin(MSGCH_WARN, jtrans("Your ring of flames is guttering out."));
...
spret_type cast_corpse_rot(bool fail)
{
    if (!you.res_rotting())
    {
        for (stack_iterator si(you.pos()); si; ++si)
        {
            ...
            {
                if (!yesno(make_stringf(jtransc("Really cast Corpse Rot while standing on "),
                                        jtransc(si->name(DESC_A))).c_str(), false, 'n'))
                {
                    canned_msg(MSG_OK);
                    return SPRET_ABORT;
                }
...
void corpse_rot(actor* caster)
{
    ...
    if (saw_rot)
        mpr(jtransc(make_stringf("You %s decay.", (you.can_smell() ? "smell" : "sense"))));
    ...
    if (cloud_count)
    {
        if (defender->is_player())
            ...
            simple_monster_message(defender->as_monster(),
                                   jtransc(" is surrounded by blessed fire!"));
    }
}
