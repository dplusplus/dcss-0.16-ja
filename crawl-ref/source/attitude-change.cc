...
bool make_god_gifts_disappear()
{
    ...
    for (monster_iterator mi; mi; ++mi)
    {
        ...
        {
            if (simple_monster_message(*mi, jtransc(" abandons you!")))
                count++;
...
bool yred_slaves_abandon_you()
{
    ...
    if (num_reclaim > 0)
    {
        if (num_reclaim == 1 && num_slaves > 1)
            simple_god_message(jtransc(" reclaims one of your granted undead slaves!"));
        else if (num_reclaim == num_slaves)
            simple_god_message(jtransc(" reclaims your granted undead slaves!"));
        else
            simple_god_message(jtransc(" reclaims some of your granted undead slaves!"));
        return true;
    }
...
static void _jiyva_convert_slime(monster* slime)
{
    ...
    if (!mons_eats_items(slime))
    {
        ...
        mprf(MSGCH_MONSTER_ENCHANT, jtransc("%s looks hungrier."),
             slime->name(DESC_THE).c_str());
    }
